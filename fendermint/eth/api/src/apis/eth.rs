    ...
    C: Client + Sync + Send,
{
    ...
    if deliver_tx.code.is_err() {
        ...
        match decode_fevm_invoke(&deliver_tx)
            .context("error decoding data from deliver_tx in query")
        {
            Ok(return_data) => Ok(et::Bytes::from(return_data)),
            Err(e) => {
                // Maybe the call isn't a method invocation on an EVM actor but a contract creation to the EAM actor.
                // This isn't something the ABI methods generated by ethers would support, and for gas estimation I
                // expect tools would use the `eth_estimateGas` endpoint, but Remix seems to use `eth_call` with a
                // deployment transaction too. It's not clear what the expected return value is, but let's try the address.
                match decode_fevm_create(&deliver_tx) {
                    Ok(create_return) => Ok(et::Bytes::from(create_return.eth_address.0)),
                    Err(_) => Err(e.into()),
                }
            }
        }
