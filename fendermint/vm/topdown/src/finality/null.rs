...
impl FinalityWithNull {
    ...
    fn propose_next_height(&self) -> Stm<Option<BlockHeight>> {
        ...
        let r = self.first_non_null_block_before(first_non_null_height - self.config.proposal_delay())?;
...
mod tests {
    ...
    use crate::{
        BlockHeight, Config, IPCParentFinality
    };
    ...
    async fn new_provider(
        ...
        for (h, p) in blocks {
            atomically_or_err(|| f.new_parent_view(h, p.clone())).await.unwrap();
        }
        f
    }
    ...
    async fn test_happy_path() {
        ...
        let f = IPCParentFinality{ height: 102, block_hash: vec![2; 32] };
        assert_eq!(
        ...
        atomically_or_err(|| provider.new_parent_view(109, None)).await.unwrap();
    }
    ...
    async fn test_no_enough_view() {
        ...
        assert_eq!(
            atomically(|| provider.next_proposal()).await,
            Some(IPCParentFinality{ height: 101, block_hash: vec![1; 32]})
        );
    }
    ...
    async fn test_with_partially_null_blocks_ii() {
        ...
        assert_eq!(
            atomically(|| provider.next_proposal()).await,
            Some(IPCParentFinality{ height: 103, block_hash: vec![3; 32]})
        );
    }
}
