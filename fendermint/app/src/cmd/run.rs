...
async fn run(settings: Settings) -> anyhow::Result<()> {
    ...
    if settings.resolver.enabled() {
        let _service = make_resolver(&settings, db.clone(), state_store.clone(), ns.bit_store)?;
    }
...
fn make_resolver(
    ...
    let service = ipc_ipld_resolver::Service::new(config, bitswap_store)
        .context("error creating IPLD Resolver")?;
...
fn to_resolver_config(settings: &Settings) -> anyhow::Result<ipc_ipld_resolver::Config> {
    ...
    if r.connection.listen_addr.is_empty() {
        bail!("Listen address is empty");
    }
    ...
    let config = Config {
        ...
        network: NetworkConfig {
            ...
            network_name: r.network.network_name.clone(),
        },
